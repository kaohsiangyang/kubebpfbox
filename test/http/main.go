...
func main() {
	...
	defer syscall.SetsockoptInt(rawSock, syscall.SOL_SOCKET, SO_DETACH_BPF, objs.SocketFilterHttp.FD())

	// Write host IP to the filter_ip map.
	const valueIPAddr uint32 = 1
	if err := objs.FilterIp.Put(ipv4ToUint32(net.ParseIP(hostIP)), valueIPAddr); err != nil {
	...
	log.Printf("%-4s %-15s %-7s -> %-15s %-7s %-10s %-5s %-5s %-6s %-3s\n",
		"Type",
		"SrcIp",
		"SrcPort",
		"DstIp",
